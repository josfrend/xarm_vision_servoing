...
class detected_object():
    def __init__(self): 
        self.pub = rospy.Publisher("/coordinate", Point, queue_size=10)
    ...
    def position_tracking(self, x, y, z):
        ...
        if self.x_rel < -1 or self.x_rel > 1:
            y_gain = self.pid_controller(0 - self.x_rel)

        if self.object_area < 65 or self.object_area > 75:
            z_gain = self.pid_controller(70 - self.object_area)
    ...
    def move_xarm(self, x, y, z):
        # Wait for the pose_plan service to become available
        rospy.wait_for_service('xarm_pose_plan')
        # Wait for the exec_plan service to become available
        rospy.wait_for_service('xarm_exec_plan')

        # Create a handle to the pose_plan service
        pose_plan_service = rospy.ServiceProxy('xarm_pose_plan', pose_plan)

        # Create a handle to the exec_plan service
        exec_plan_service = rospy.ServiceProxy('xarm_exec_plan', exec_plan)

        # Create a request object for path planning
        #plan_request = pose_plan_service.request_class
        #plan_response = pose_plan_service.response_class

        # Set the target position and orientation values for planning
        target_pose = Pose()
        ...
        #plan_request = target_pose

        # Call the pose_plan service to get the planned path
        pose_plan_service.call(target_pose)

        # Call the exec_plan service
        exec_plan_service.call(True)

        # if plan_response.success:
        #     rospy.loginfo('Path planning completed.')

        #     # Create a request object for path execution
        #     execute_request = exec_plan_service.request_class

        #     # Set the planned path obtained from planning
        #     execute_request.path = plan_response.planned_path

        #     # Call the exec_plan service to execute the path
        #     execute_response = exec_plan_service(execute_request)

        #     if execute_response.success:
        #         rospy.loginfo('Path execution completed.')
        #     else:
        #         rospy.loginfo('Path execution failed.')
    ...
    def run(self, xArm_x, xArm_y, xArm_z):
        ...
        if len(contours) > 0:
            ...
            # Muestra las coordenadas relativas en la ventana
            #cv2.putText(frame, f'({x_rel}, {y_rel})', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

            # Muestra el área del objeto en la ventana
            #cv2.putText(frame, f'Area: {object_area}', (x, y + h + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
